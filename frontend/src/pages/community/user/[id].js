...
export default function CommunityUserPage() {
  ...
  const openPost = (p) => router.push(`/community/post/${p.id}`)
  ...
  return (
    <div className="co-doc">
      <header className="co-doc-bar">
        ...
        <h1 className="co-doc-title">{t(lang, 'proTitle')}</h1>
      ...
      <div className="co-doc-col">
        {id ? (
          <CommunityProfileView id={id} lang={lang} me={me} onOpenPost={openPost} />
        ) : (
      ...
      <style jsx global>{`
        ...
        .co-doc-btn {
          ...
          padding: 8px 10px 8px 4px; font-family: var(--font-rounded);
          font-size: 13.5px; font-weight: 700; cursor: pointer; border-radius: 999px; transition: all .15s;
        }
        .co-doc-btn:hover { background: rgba(255, 255, 255, .08); color: #fff; }
        .co-doc-title {
          margin: 0; text-align: center; font-family: var(--font-rounded);
        ...
        @media (min-width: 1024px) { .co-doc-col { padding-bottom: 120px; } }
        .cu-empty, .co-doc-empty { text-align: center; padding: 40px 0; font-family: var(--font-rounded); font-size: 13px; color: rgba(255, 255, 255, .45); }
      `}</style>
    </div>
  )
}
